...
sub trim{
	...
	`java -jar /usr/local/bin/trimmomatic-0.33.jar PE -threads $threads -phred33 ./fastq/${sample}_1.fastq.gz ./fastq/${sample}_2.fastq.gz ${sample}_1_trimmed_paired.txt ${sample}_1_trimmed_unpaired.txt ${sample}_2_trimmed_paired.txt ${sample}_2_trimmed_unpaired.txt LEADING:3 TRAILING:3 SLIDINGWINDOW:4:20 MINLEN:36 2>> $sample.log`;
